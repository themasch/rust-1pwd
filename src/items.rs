...
impl<E, D: Decoder<E>> Decodable<D, E> for ContentItem {

    fn decode(d: &mut D) -> Result<ContentItem, E> {
...
mod unittest {
    ...
    fn decode_single_content_item() {
        ...
        let ci: ContentItem = json::decode(encoded.as_slice()).unwrap();
    ...
    fn decode_content_item_list() {
        ...
        let ci: Vec<ContentItem> = json::decode(encoded.as_slice()).unwrap();
